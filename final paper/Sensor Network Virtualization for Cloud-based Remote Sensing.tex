...
% *** with production work. IEEE's  choices and paper sizes can       ***
...
\begin{abstract}
In real world, the actual network infrastructure can be dynamic. With the help of network virtualization, we are able to performan remote sensing task regardless of physicial network changes. For a given network, if we tried to perform any task, the first crucial step is be able to spread the task requirement in the network in an effecient manner. In short, task message need to be broadcasted quickly. In this paper, we used ns-3 to implement gossip protocol.  
...
Based on the wired mesh network, now we could replace that with a wireless mesh network. The topology is the same as the wired mesh network. Depending on different wireless technologies, we could define different connection rate. For example, zigbee and 6lowpan has the same maximum speed of 250Kbps. But WiFi Direct connection speed could be up to 250Mbps. Then we could plug in random variables representing random locations of devices, random packet drops of the network and power consumption of devices. Finally, we could implement the protocol to evaluate its performance.

\section{Related Work}
In this section, we introduced the topic of Virtual Networks formed using a subset of a large group of IoT devices. Relevant papers covering the topic of Virtual Networks are summarized. In addition, one paper about WiFi Direct technology is summarized as well.
...
We would like to thank Dr. Bechir Hamdaoui for his advising. We also would like to thank Sherif Abelwahab for his advising.
...
%[1] Luigi Atzori, Antonio Iera, Giacomo Morabito, The Internet of Things: A survey, Computer Networks, Volume 54, Issue 15, 28 October 2010, Pages 2787-2805, ISSN 1389-1286,
%[2] Jayasumana, Anura P., Qi Han, and Tissa H. Illangasekare. "Virtual sensor networks-A resource efficient approach for concurrent applications." Information Technology, 2007. ITNG'07. Fourth International Conference on. IEEE, 2007.
%[3] Chowdhury, NM Mosharaf Kabir, and Raouf Boutaba. "A survey of network virtualization." Computer Networks 54.5 (2010): 862-876.
%[4] Abdelwahab, Sherif, Bechir Hamdaoui, and Mohsen Guizani. "Cloud-Assisted Remote Sensor Network Virtualization for Distributed Consensus Estimation." arXiv preprint arXiv:1501.03547 (2015).
%[5] Camps-Mur, D.; Garcia-Saavedra, A.; Serrano, P., "Device-to-device communications with Wi-Fi Direct: overview and experimentation," Wireless Communications, IEEE , vol.20, no.3, pp.96,104, June 2013.
