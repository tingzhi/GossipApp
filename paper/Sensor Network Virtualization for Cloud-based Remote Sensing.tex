...
Gossip protocol is an computer comunication protocol which inspired by the social activity -- gossip. This protocol suit for the synchronize the message in the system which do not need the realtime synchronization. It provide O(logn) time to synchronize the message to the network, n means the number of nodes in the network. There are three packet type for the message protocol: data packet, ACK packet and request packet. The data packet contains the message from a certain source node who want to spread this message to the whole network. ACK packet is a packet to notice the node that the destination alread contain the mesaage. The request packet is the packet which request the message from destination node. There are two state for each node, running and stop.
...
\begin{figure}
 ...
 switch(state):
        	...
        	if message is not null:
            	every 1 millisecond:
                	find a random neighbor
                 	send data packet to random neightbor
                  	if receive a packet:
                      	if pakcet is ACK:
                          	state <- stop
         	else:
           		if receive a packet:
                	if packet is a data packet:
                    	update the message to the content of data packet
                every 5 second:
                	find a random neighbor
                    send a request packet to the neighbor
                    if receive a packet:
                    	if packet is a data packet:
                        	update the message to the content of data packet 
        ...
        case stop:
			if receive a packet:
                	if pakcet is request:
                    	send data to the node who send the packet
                    if packet is data:
                    	send ACK to the node who send the packet
\end{verbatim}
...
In further work, various alterations to the gossip protocol can be covered and examined.
