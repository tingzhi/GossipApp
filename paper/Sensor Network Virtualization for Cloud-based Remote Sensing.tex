...
Another interesting observation is that accross these statistical analysis, the case of 100 nodes always has different behavior than other cases. We wondered if that has anything to do with not eough experiment trials. So we run each topology file of 100 nodes case one time, which is defalut in our performance evaluation setting, and 50 times just to see if the standard deviation would decrease as number of experiment trails increase. As we can see in fig.~\ref{fig:100time} and fig.~\ref{fig:100hops}. There is no sign that in this case, the standard deviation would decrease as the number of experiment trials increases.

\begin{figure}
	\centering
	\includegraphics[width=3in]{figs/100_time}
	\caption{Spread Time Under 1 and 50 Run Times for 100 Node Cases}
	\label{fig:100time}
\end{figure}

\begin{figure}
	\centering
	\includegraphics[width=3in]{figs/100_hops}
	\caption{Maximum Hops Under 1 and 50 Run Times for 100 Node Cases}
	\label{fig:100hops}
\end{figure}
